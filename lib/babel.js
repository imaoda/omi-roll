  ...
  _ref = _asyncToGenerator(function* (opts) {
    ...
    function createStream(src) {
      return _vinylFs.default.src(src, {
        allowEmpty: true,
        base: srcPath
      }).pipe((0, _gulpIf.default)(f => /\.tsx?$/.test(f.path), (0, _gulpTypescript.default)(getTSConfig()))).pipe((0, _gulpIf.default)(f => /\.jsx?$/.test(f.path), _through.default.obj((file, env, cb) => {
    ...
    return new Promise(resolve => {
      createStream([(0, _path.join)(srcPath, '**/*'), `!${(0, _path.join)(srcPath, '**/fixtures/**/*')}`, `!${(0, _path.join)(srcPath, '**/*.mdx')}`, `!${(0, _path.join)(srcPath, '**/*.+(test|e2e|spec).+(js|jsx|ts|tsx)')}`]).on('end', () => {
        if (watch) {
